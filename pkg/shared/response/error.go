...
// is augmented with an `errors` field of `response.ErrorDetail` objects that
...
// `response.ErrorDetailer` interface, then it is used, otherwise the error
// string is used as the error detail message.
//
//	err := &ErrorModel{ /* ... */ }
//	err.Add(&response.ErrorDetail{
//		Message: "expected boolean",
//		Location: "body.friends[1].active",
//		Value: 5
//	})
...
// `response.Error400BadRequest` with additional headers.
...
// error response utility functions, like `response.Error400BadRequest`.
//
...
//	}
//
//	response.NewError = func(status int, msg string, errs ...error) StatusError {
//		return &MyError{
//			status:  status,
//			Message: msg,
//			Errors:  errs,
...
var NewErrorWithContext = func(_ context.Context, status int, msg string, errs ...error) StatusError {
